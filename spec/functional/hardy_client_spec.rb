...
describe 'harvest hardy client' do
  ...
  it "waits the alloted time out when over rate limit" do
    stub_request(:get, /\/clients/).to_return(
      {
        :status => ['503', 'Rate Limited'],
  ...
  it "retries after known errors" do
    stub_request(:get, /\/rate_limit_status/).to_return(
      {
        :body => '{' \
          '"lockout_seconds":2,' \
          '"last_access_at":"2011-06-18T17:13:22+00:00",' \
          '"max_calls":100,' \
          '"count":1,' \
          '"timeframe_limit":15' \
        '}'
    ...
    ).times(2).then.to_return({:body => "[]", :status => 200})
    hardy_harvest.clients.all.should == []

    stub_request(:get, /\/clients/).to_raise(
      Net::HTTPError.new("custom error", "")
    ).times(2).then.to_return({:body => "[]", :status => 200})
    hardy_harvest.clients.all.should == []

    stub_request(:get, /\/clients/).to_return(
      {:status => ['502', 'Bad Gateway']}
    ).times(6).then.to_return({:body => "[]", :status => 200})
