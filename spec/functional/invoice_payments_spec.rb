...
describe 'harvest invoice payments' do
  it 'allows retrieving existing invoice payments' do
    cassette('invoice_payment1') do
      ...
      invoice = harvest.invoices.create(
        FactoryGirl.attributes_for(:invoice, :client_id => client.id)
      )

      payment = Harvest::InvoicePayment.new(
  ...
  it 'allows adding, and removing invoice payments' do
    cassette('invoice_payment2') do
      ...
      payment1 = Harvest::InvoicePayment.new(
        FactoryGirl.attributes_for(
          :invoice_payment,
          :invoice_id => invoice.id,
          :amount => half_amount
      ...
      payment2 = Harvest::InvoicePayment.new(
        FactoryGirl.attributes_for(
          :invoice_payment,
          :invoice_id => invoice.id,
          :amount => half_amount
      ...
      harvest.invoice_payments.all(invoice).each do |ip|
        harvest.invoice_payments.delete(ip)
      end
