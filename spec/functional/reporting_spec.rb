  ...
  it 'allows project and people entry reporting' do
    cassette("reports1") do
      ...
      project1 = harvest.projects.create(
        name: "Reporting Project1",
        client_id: client.id
      )
      project2 = harvest.projects.create(
        name: "Reporting Project2",
        client_id: client.id
      )
      ...
      harvest.reports.time_by_project(
        project1,
        Time.utc(2009, 12, 20),
        Time.utc(2009,12,30)
      ).first.should == entry1
      ...
      harvest.reports.time_by_user(
        ...
        Time.utc(2009, 12, 20),
        Time.utc(2009,12,30)
      ).map(&:id).should == [entry1, entry2].map(&:id)
      ...
      entry3  = harvest.time.create(
        notes: "test entry for checking updated_since",
        hours: 5,
        spent_at: "2009/12/15",
        task_id: task1.id,
        project_id: project1.id,
      ...
      entries = harvest.reports.time_by_user(
        user,
        Time.utc(2009, 12, 10),
        Time.utc(2009,12,30),
        {
          project: project1,
          updated_since: Time.parse(entry3.updated_at) - 1
        }
      )
      ...
      harvest.reports.projects_by_client(client).map(&:id).to_set.
        should == [project1, project2].map(&:id).to_set
  ...
  it 'allows expense reporting' do
    cassette("reports2") do
      ...
      project = harvest.projects.create(
        name: "Expense Reporting Project",
        client_id: client.id
      )
      ...
      harvest.reports.expenses_by_project(
        project,
        Time.utc(2009, 12, 20),
        Time.utc(2009,12,30)
      ).first.should == expense
      ...
      my_user = harvest.users.all.detect do |u|
      ...
      my_project = harvest.projects.create(
        name: "Travel Expense Project",
        client_id: client.id
      )
      harvest.reports.expenses_by_project(
        my_project,
