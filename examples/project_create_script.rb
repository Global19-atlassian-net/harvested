...
# Author(s): Mark Rickert (mjar81@gmail.com) / Skookum Digital Works -
#   http://skookum.com
#
# Description: This example script takes user input from the command line and
# creates a project based the selected options. It then assigns tasks from
# Harvest to the project based on an array. After the tasks are added, it addes
# all the currently active users to the project.
...
client_search_results = harvest.clients.all.select do |c|
  c.name.downcase.include?(cname)
end
...
puts "\nWhat sorts of tasks does this project need? Type \"p\" for project " \
  "tasks or \"s\" for sales tasks.\n"
...
puts "You can find the project here: " \
  "http://#{subdomain}.harvestapp.com/projects/#{project.id}/edit\n\n"
