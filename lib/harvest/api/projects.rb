module Harvest
  module API
    class Projects < Base
      ...
      # Retrieves all projects. Note: this requires project manager or
      ...
      def all(*)
        ...
        raise NotFound.new(
          e.response,
          e.params,
          "Do you have sufficient privileges? If not, consider using " \
          "time.trackable_projects instead."
        )
      end
      ...
      def create_task(project, task_name)
        response = request(
          :post,
          credentials,
          "/projects/#{project.to_i}/task_assignments/add_with_create_new_task",
          :body => {"task" => {"name" => task_name}}.to_json
        )
        id = response.headers["location"].match(/\/.*\/(\d+)\/.*\/(\d+)/)[1]
        find(id)
      end

      # Deactivates the project. Does nothing if the project is already
      # deactivated
      #
      ...
      def deactivate(project)
        if project.active?
          request(
            :put,
            credentials,
            "#{api_model.api_path}/#{project.to_i}/toggle",
