    ...
    class InvoicePayments < Base
      ...
      def all(invoice)
        response = request(
          :get,
          credentials,
          "/invoices/#{invoice.to_i}/payments"
        )
      ...
      def find(invoice, payment)
        response = request(
          :get,
          credentials,
          "/invoices/#{invoice.to_i}/payments/#{payment.to_i}"
        )
      ...
      def create(payment)
        ...
        response = request(
          :post,
          credentials,
          "/invoices/#{payment.invoice_id}/payments",
      ...
      def delete(payment)
        request(
          :delete,
          credentials,
          "/invoices/#{payment.invoice_id}/payments/#{payment.to_i}"
        )
        payment.id
      end
