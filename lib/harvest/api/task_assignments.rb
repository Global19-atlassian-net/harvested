    ...
    class TaskAssignments < Base
      def all(project, query = {})
        response = request(
          :get,
          credentials,
          "/projects/#{project.to_i}/task_assignments",
          {query: query}
        )
      ...
      def find(project, id)
        response = request(
          :get,
          credentials,
          "/projects/#{project.to_i}/task_assignments/#{id}"
        )
      ...
      def create(task_assignment)
        ...
        response = request(
          :post,
          credentials,
          "/projects/#{task_assignment.project_id}/task_assignments",
          :body => task_assignment.task_as_json.to_json
        )
      ...
      def update(task_assignment)
        ...
        request(
          :put,
          credentials,
          "/projects/#{task_assignment.project_id}" \
          "/task_assignments/#{task_assignment.to_i}",
          :body => task_assignment.to_json
        )
