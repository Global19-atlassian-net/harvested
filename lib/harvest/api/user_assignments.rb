    ...
    class UserAssignments < Base
      ...
      def all(project, query = {})
        response = request(
          :get,
          credentials,
          "/projects/#{project.to_i}/user_assignments",
          {query: query}
        )
      ...
      def find(project, id)
        response = request(
          :get,
          credentials,
          "/projects/#{project.to_i}/user_assignments/#{id}"
        )
      ...
      def create(user_assignment)
        ...
        response = request(
          :post,
          credentials,
          "/projects/#{user_assignment.project_id}/user_assignments",
          :body => user_assignment.user_as_json.to_json
        )
      ...
      def update(user_assignment)
        ...
        request(
          :put,
          credentials,
          "/projects/#{user_assignment.project_id}" \
            "/user_assignments/#{user_assignment.id}",
          :body => user_assignment.to_json
        )
      ...
      def delete(user_assignment)
        request(
          ...
          credentials,
          "/projects/#{user_assignment.project_id}" \
            "/user_assignments/#{user_assignment.to_i}"
        )
        user_assignment.id
      end
    end
