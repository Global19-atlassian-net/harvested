    ...
    class InvoiceMessages < Base
      ...
      def all(invoice)
        response = request(
          :get,
          credentials,
          "/invoices/#{invoice.to_i}/messages"
        )
      ...
      def find(invoice, message)
        response = request(
          :get,
          credentials,
          "/invoices/#{invoice.to_i}/messages/#{message.to_i}"
        )
      ...
      def create(message)
        ...
        response = request(
          :post,
          credentials,
          "/invoices/#{message.invoice_id}/messages", :body => message.to_json
        )
      ...
      def delete(message)
        request(
          :delete,
          credentials,
          "/invoices/#{message.invoice_id}/messages/#{message.to_i}"
        )
        message.id
      end
      ...
      # Create a message and mark an open invoice as closed (writing an
      # invoice off)
      #
      # @param [Harvest::InvoiceMessage] The message you want to send
      # @return [Harvest::InvoiceMessage] The sent message
