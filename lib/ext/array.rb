...
unless Array.respond_to?(:wrap)
  class Array
    ...
    # +nil+, but <tt>Arraw.wrap</tt> returns such a +nil+ right away.
    # * If the returned value from +to_ary+ is neither +nil+ nor an
    # +Array+ object, <tt>Kernel#Array</tt> raises an exception, while
    # <tt>Array.wrap</tt> does not, it just returns the value.
    # * It does not call +to_a+ on the argument, though special-cases +nil+
    ...
    #   [*object]
    #
    # which returns <tt>[nil]</tt> for +nil+, and calls to
    # <tt>Array(object)</tt> otherwise.
    #
    # Thus, in this case the behavior is different for +nil+, and the
    # differences with <tt>Kernel#Array</tt> explained above apply to the rest
    # of +object+s.
