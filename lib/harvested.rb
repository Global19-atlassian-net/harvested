...
%w(
  crud
  activatable
).each {|a| require "harvest/behavior/#{a}"}
...
module Harvest
  class << self
    ...
    # Creates a hardy client that will retry common HTTP errors it encounters
    # and sleep() if it determines it is over your rate limit
    #
    # == Options
    # * Basic Authentication
    #   * +:subdomain+ - Your Harvest subdomain
    ...
    #   * +:access_token+ - An OAuth 2.0 access token
    #
    # * +:retry+ - How many times the hardy client should retry errors. Set
    #   to +5+ by default.
    #
    # == Examples
    ...
    #
    # == Rate Limits
    # The hardy client will make as many requests as it can until it detects
    # it has gone over the rate limit. Then it will +sleep()+ for the how ever
    # long it takes for the limit to reset. You can find more information about
    # the Rate Limiting at http://www.getharvest.com/api
    #
    # @return [Harvest::HardyClient] a Harvest::Base wrapped in a
    #   Harvest::HardyClient
    # @see Harvest::Base
    def hardy_client(
      subdomain: nil,
